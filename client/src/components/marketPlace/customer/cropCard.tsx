...
function CropCard({ className }: { className: string }) {
  ...
  const handleCartItems = async (item: CropsMarketPlaceProps) => {
    if (getUserDetails().userDetails.type !== "customer") {
      return handleToast({
        message: "You are not authorized to add items to cart",
        type: "error",
      });
    }
  ...
  return (
    <div
      className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3  gap-6 p-6 ${className}`}>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  ...
                  <span className="text-sm text-gray-600">per 50kg</span>
                </div>
